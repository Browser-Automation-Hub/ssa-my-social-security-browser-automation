# Contributing to SSA my Social Security Browser Automation
...
- **Broken selectors**: SSA my Social Security updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real SSA my Social Security instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.SSA_PORTAL_URL}/path/to/section`);
...
When you find working selectors for SSA my Social Security, document them:

```javascript
// SSA my Social Security selectors (verified YYYY-MM-DD)
...
SSA my Social Security updates its UI periodically. When selectors break:
