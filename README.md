...
[![npm](https://img.shields.io/npm/v/@browser-automation-hub%2Fssa-my-social-security-browser-automation.svg)](https://www.npmjs.com/package/@browser-automation-hub/ssa-my-social-security-browser-automation)

> Automate SSA my Social Security — the reliable way to interact with SSA Portal programmatically, with or without an official API.
...
<!-- keywords: social security automation, ssa portal automation, ssa my account automation, social security benefits automation, ssa api alternative, social security verification automation -->

## What This Is

**SSA my Social Security** (Government) is notoriously difficult to automate via its official API — limited endpoints, complex authentication (Login.gov / ID.me), and browser-only workflows make traditional API integration a pain.

This project gives you a **complete browser automation scaffold** for SSA my Social Security using Puppeteer (self-hosted, open source) or [Anchor Browser](https://anchorbrowser.io) (cloud, managed, production-ready).

This system requires **MFA** (ID.me / Login.gov OTP). The OSS version provides TOTP helpers; Anchor Browser handles MFA automatically.

## Quick Start

```bash
git clone https://github.com/Browser-Automation-Hub/ssa-my-social-security-browser-automation.git
cd ssa-my-social-security-browser-automation
npm install
cp .env.example .env
...
- Benefits verification for financial institutions
- HR/payroll firms verifying SSA records
- Elder care agencies managing client SSA documents
- Tax professionals retrieving SSA-1099s at scale
...
- SSA my Social Security account with appropriate permissions
...
```env
SSA_PORTAL_URL=https://www.ssa.gov/myaccount/
SSA_PORTAL_USERNAME=your-username
SSA_PORTAL_PASSWORD=your-password
...
async function main() {
  ...
  const result = await login_ssa(page, { /* options */ });
  console.log(result);
}
...
3. **MFA / TOTP** (ID.me / Login.gov OTP) — generates TOTP codes via `otpauth` library
4. **Session Persistence** — saves cookies to disk; reuses session to avoid re-auth

### Handling ID.me / Login.gov OTP MFA
...
SSA my Social Security provides no public API. All workflows are web-portal only with strict session requirements.
...
- Handles ID.me / Login.gov OTP MFA automatically
...
> These CSS selectors were observed in SSA my Social Security web interfaces. Enterprise applications update their UIs — verify against your specific instance and submit PRs when selectors break.

> 🔍 Selector reference not yet documented for SSA my Social Security. [Contribute selectors via PR](https://github.com/Browser-Automation-Hub/ssa-my-social-security-browser-automation/pulls).
...
2. Document new selectors (they break when SSA Portal updates its UI)
...
*Built with ❤️ for developers who need to automate SSA my Social Security without wrestling with its API limitations. Powered by [Anchor Browser](https://anchorbrowser.io) for cloud-scale automation.*
