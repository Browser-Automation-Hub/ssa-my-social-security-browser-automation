/**
 * anchor-cloud.js — AnchorBrowser cloud example for SSA my Social Security
 *
...
const { login_ssa, get_benefit_verification_letter } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting SSA my Social Security automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_ssa(page, {});
      log('Action 1 result:', data1);

      const data2 = await get_benefit_verification_letter(page, {});
