/**
 * auth.js — Authentication handler for SSA my Social Security
 * Supports: SSO (Login.gov / ID.me), MFA (ID.me / Login.gov OTP)
 *
...
const BASE_URL = process.env.SSA_PORTAL_URL || 'https://www.ssa.gov/myaccount/';
const USERNAME = process.env.SSA_PORTAL_USERNAME;
const PASSWORD = process.env.SSA_PORTAL_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set SSA_PORTAL_USERNAME and SSA_PORTAL_PASSWORD in .env`);
  }

  console.log('Logging in to SSA my Social Security...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
